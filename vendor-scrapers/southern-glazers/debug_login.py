#!/usr/bin/env python3
"""Debug: Examine Southern Glazer's login page and test auto-login."""
import asyncio
import json
import os
...
async def main():
    async with async_playwright() as pw:
        ...
        # Screenshot
        await page.screenshot(path="/tmp/sg_login_page.png", full_page=True)
        print(f"\nScreenshot saved to /tmp/sg_login_page.png")

        print(f"\n{'=' * 60}")
        print("STEP 2: Try auto-login")
        print("=" * 60)

        user = os.environ.get("SG_CHATHAM_USER", "")
        pw_val = os.environ.get("SG_CHATHAM_PASS", "")
        print(f"  User: {user}")
        ...
        if user and pw_val:
            ...
            if email_filled and pw_filled:
                ...
                await page.screenshot(path="/tmp/sg_after_login.png", full_page=True)
                print(f"\n  Screenshot saved to /tmp/sg_after_login.png")
